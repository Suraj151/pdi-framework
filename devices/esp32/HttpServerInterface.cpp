...
CallBackVoidArgFn HttpServerInterface::UriToHandlerMap::notFoundHandler = nullptr;
    ...
    m_client(nullptr)
{
    ...
    m_storagePath = "/var/www/static/"; // Default storage path for static files
}
 ...
 */
void HttpServerInterface::begin(uint16_t port){
    ...
    __i_fs.createDirectory(m_storagePath.c_str());
    #endif
}
 ...
 */
void HttpServerInterface::handleClient(){
 ...
 * close
 */
void HttpServerInterface::close(){
    closeClient();
    if( nullptr != m_server ){
        m_server->close();
    }
 ...
 */
void HttpServerInterface::on(const pdiutil::string &uri, CallBackVoidArgFn handler){
 ...
 */
void HttpServerInterface::onNotFound(CallBackVoidArgFn fn){
 ...
 */
pdiutil::string HttpServerInterface::arg(const pdiutil::string &name) const {
 ...
 */
bool HttpServerInterface::hasArg(const pdiutil::string &name) const{
 ...
 */
void HttpServerInterface::collectHeaders(const char *headerKeys[], const size_t headerKeysCount){
 ...
 */
pdiutil::string HttpServerInterface::header(const pdiutil::string &name) const {
 ...
 */
bool HttpServerInterface::hasHeader(const pdiutil::string &name) const{    
 ...
 * add Header
 */
void HttpServerInterface::addHeader(const pdiutil::string &name, const pdiutil::string &value){
 ...
 */
void HttpServerInterface::setStoragePath(const pdiutil::string &storagepath) {
 ...
 * send
 */
void HttpServerInterface::send(int code, const char *content_type, const char *content){
    if( nullptr != content_type ){
        sendResponse(code, content_type, content);
    }else{
        sendResponse(code, getMimeTypeString(MIME_TYPE_TEXT_HTML), "");
    }
}
 ...
 */
void HttpServerInterface::parseRequest(){
    ...
    __i_dvc_ctrl.yield(); // Yield to allow other tasks to run
        ...
        while (1) {
            ...
            if (part.find(boundary) != pdiutil::string::npos) {
                ...
                if (part.find("Content-Disposition") != pdiutil::string::npos) {
                    ...
                    pdiutil::string argtype = getMimeTypeString(MIME_TYPE_TEXT_PLAIN);
                    ...
                    if( argfilename.empty() ){
                        ...
                        if(__i_fs.isFileExist(tempFilePath.c_str())){
                            // If the file already exists, delete it
                            __i_fs.deleteFile(tempFilePath.c_str());
                        }
                        ...
                        while (1) {
                            ...
                            if(++filewritecounter == 2){
                                ...
                                __i_fs.writeFile(tempFilePath.c_str(), lastread.c_str(), lastread.length(), true);
                                filewritecounter = 0;
                            }
                            #else
 ...
 */
void HttpServerInterface::prepareResponseHeader(pdiutil::string& _header, int code, const char *content_type, uint32_t content_length){
 ...
 */
void HttpServerInterface::sendResponse(int code, const char *content_type, const char *content){

    if (!m_client || !content_type || !content) {
        return; // Client not initialized
    }

    pdiutil::string response;
    prepareResponseHeader(response, code, content_type, strlen(content));
 ...
 */
bool HttpServerInterface::handleStaticFileRequest(){
    ...
    if (m_clientRequest.method == "GET" || m_clientRequest.method == "HEAD") {
        ...
        mimetype_t filetype = __i_fs.getFileMimeType(filePath.c_str());
        ...
        if (__i_fs.isFileExist(filePath.c_str())) {
            ...
            pdiutil::string contentType = getMimeTypeString(filetype);

            // Add Content-Disposition header to force download
            m_responseHeaders.clear();
            pdiutil::string content_disposition_value = "attachment; filename=\"" + __i_fs.basename(filePath.c_str()) + "\"";
            addHeader(HTTP_HEADER_KEY_CONTENT_DISPOSITION, content_disposition_value);

            pdiutil::string response;
            prepareResponseHeader(response, HTTP_RESP_OK, getMimeTypeString(filetype), __i_fs.getFileSize(filePath.c_str()));

            // Send the response headers
            sendPacket(m_client, (uint8_t *)response.c_str(), response.length());

            // Send the response body
            int iStatus = __i_fs.readFile(filePath.c_str(), 400, [&](char* data, uint32_t size)->bool{
 ...
 */
void HttpServerInterface::closeClient() {
    if (m_client) {
        m_client->close();
        delete m_client;
        m_client = nullptr;
...
HttpServerInterface __i_http_server;
