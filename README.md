...
boot
 ...
 │     └─ (if WiFi) new TcpClientInterface
 │
setup()
 │
...
loop()
 ...
 └─ PdiStack.serve()                        (src/PdiStack.cpp)
       ...
       ├─ __i_cooperative_scheduler.run()                  (if contextual)
...
└── threading/                    Optional — only if the port provides preemptive
    └── ...                       and/or cooperative execution contexts
```
...
| `iExecution`, `iMutex`, `iCondvar`, `iContext` | Optional | `ENABLE_CONTEXTUAL_EXECUTION` | Needed only for cooperative/preemptive task modes |
...
devices/DeviceConfig.h              cascades into ENABLE_* feature flags
        ...
        ▼
src/config/Config.h                 (everything in src/* sees the flags)
        │
        ▼
...
Two files need a matching entry when adding a device: [src/interface/pdi.h](src/interface/pdi.h) and the new branch in [devices/DeviceConfig.h](devices/DeviceConfig.h) for any per-device limits (e.g. `MAX_DIGITAL_GPIO_PINS`, `MAX_DB_TABLES`).
...
| `__i_cooperative_scheduler`, `__i_preemptive_scheduler` | [devices/esp8266/threading/](devices/esp8266/threading/) | `ENABLE_CONTEXTUAL_EXECUTION` |
...
Cooperative / preemptive task modes require five additional pieces, currently only implemented for ESP8266 under [devices/esp8266/threading/](devices/esp8266/threading/):

- An `iExecution`-derived scheduler (one for cooperative, one for preemptive).
- An `iContext` implementation that saves/restores CPU state (e.g. `XtensaContext`).
- An `iMutex` and `iCondvar` pair appropriate for the mode.
- A periodic tick source for the preemptive lane (e.g. `XtensaTimer`).

If a port omits this layer, contextual tasks are simply unavailable — inline tasks continue to work because they share the `loop()` stack. Section [5. Task Scheduler] details the API and the trade-offs.
...
1. **Create the folder and umbrella header**
   ```
   devices/myboard/
...
│   ├── iServerInterface.h         iTcpServerInterface + iHttpServerInterface
...
| `iInstanceInterface` | [src/utility/iInstanceInterface.h](src/utility/iInstanceInterface.h) | Device | Services that need fresh TCP/FS instances (MQTT pool, SSH, OTA) | `getNewTcpClientInstance`, `getNewTcpServerInstance`, `getFileSystemInstance`, `getUtilityInstance` |
...
| `iHttpServerInterface` | same file | Device (or `impl/HttpServerInterfaceImpl`) | `WebServer` | Routing (`on`, `onNotFound`), args/headers, `send(code, mime, body, chunked)` |
...
| `iFileSystemInterface` | [modules/storage/iFileSystemInterface.h](src/interface/pdi/modules/storage/iFileSystemInterface.h) | Device (often via [`FileSystemInterfaceImpl`](src/interface/pdi/impl/modules/storage/FileSystemInterfaceImpl.h)) | SSH/SFTP, `fread`/`fwrite`/`ls`/`cd`/`mv`/`cp`/`rm`/`mkd`/`mkf` CLI | 37 virtuals: file & directory CRUD, traversal, stat. Constructed with an `iStorageInterface&` |
...
|---|---|---|
| [`HttpServerInterfaceImpl`](src/interface/pdi/impl/middlewares/HttpServerInterfaceImpl.h) | A protocol-correct HTTP/1.1 server built on top of `iTcpServerInterface` + `iTcpClientInterface` | Almost never — only if the SDK provides a measurably better native HTTP server |
...
This is how the framework keeps the porting effort small: a new device only needs raw `iTcpServerInterface` + `iStorageInterface`, and the rest of the stack (HTTP server, file system) is inherited.
...
|---|---|---|---|
| **Device tier** | [devices/DeviceConfig.h](devices/DeviceConfig.h), [devices/DeviceSetup.h](devices/DeviceSetup.h) | `DEVICE_*` selector, `ENABLE_*` feature flags, per-board limits (`MAX_DB_TABLES`, `MAX_DIGITAL_GPIO_PINS`, …) | Integrator: "what board, what features" |
...
| `ENABLE_STORAGE_SERVICE` | yes (non-uno) | `StorageConfig.h` + filesystem | `iStorageInterface` + `iFileSystemInterface` | medium (FS overhead) |
...
| `ENABLE_CONTEXTUAL_EXECUTION` | off (esp8266 only) | Cooperative + preemptive scheduler lanes | `iExecution`, `iMutex`, `iCondvar`, `iContext` + scheduler globals | high (per-task stacks) |
| `ENABLE_TIMER_TASK_SCHEDULER` | off | Timer-backed task scheduler variant | Device timer | depends |
...
| [HttpConfig.h](src/config/HttpConfig.h) | — (transient `http_param_t`) | `HTTP_CLIENT_BUF_SIZE`, request size limits |
...
- `ENABLE_CONTEXTUAL_EXECUTION` ⇒ the port supplies the full threading interface family ([§3.3.6](#3-portable-interfaces)) — currently esp8266 only.
...
| **Concurrent demo** (esp8266) | Default + `ENABLE_CONTEXTUAL_EXECUTION` | — |
...
__i_cooperative_scheduler.run();        // cooperative lane
...
| Promote a task to a contextual lane | `scheduleUnderExecSched(sched, id, mode, stackdepth)` | void | Requires `ENABLE_CONTEXTUAL_EXECUTION` |
...
| Event contract | Emits `EVENT_WIFI_CONNECTED` / `EVENT_WIFI_DISCONNECTED`; consumers (MQTT, OTA, Email, IoT) react |
...
Full breakdown lives in [§9. Web Server](#9-web-server-pending) — it has its own router, middleware chain, controllers, views, and session handler that don't fit the `ServiceProvider` shape.
...
| Built-in commands | `ls`, `cd`, `pwd`, `mkd`, `mkf`, `mv`, `cp`, `rm`, `fread`, `fwrite`, `cls`, `gpio`, `net`, `srvc`, `scht`, `ssh`, `iot`, `auth`, `reboot`, `watch` — full reference in [README Terminal table](#terminal) and [§10](#10-command-line--terminal-pending) |
...
| `EVENT_WIFI_CONNECTED` / `_DISCONNECTED` | `__wifi_service` | MQTT (re/dis-connect), OTA (suspend), IoT (suspend) |
| `EVENT_INTERNET_AVAILABLE` / `_LOST` | `__wifi_service` connectivity poller | OTA, IoT, Email |
...
|---|---|---|---|---|
| HTTP | [transports/http/](src/transports/http/) | `Http_Client` | `OtaServiceProvider`, `DeviceIotServiceProvider`, `GpioServiceProvider` (HTTP-post mode), `WebServer` indirectly | HTTP/1.1 (default), 1.0 supported; placeholders for 2/3 |
...
There is no `Http_Server` class in `transports/` — the **server** side lives at the interface layer ([`iHttpServerInterface`](src/interface/pdi/middlewares/iServerInterface.h)) with a portable default in [src/interface/pdi/impl/middlewares/HttpServerInterfaceImpl.{h,cpp}](src/interface/pdi/impl/middlewares/HttpServerInterfaceImpl.h). This split is intentional: the client is reused stand-alone; the server is plumbed through the `WebServer` ([§9](#9-web-server-pending)) and never speaks bare HTTP.
...
| `fread`, `fwrite` | [FileFSCommand.h](src/service_provider/cmd/commands/FileFSCommand.h) | `__i_fs` (`fwrite` is multi-iteration with ESC to finalize) |
...
Most commands are 1-2 hundred lines; the only big one is `FileFSCommand` (311 lines) because `fwrite` carries a state machine for line-by-line entry, ESC handling, and append-mode file persistence.
...
| Heap | `pdiutil::vector` / `pdiutil::string` storage, `m_client = new TcpClientInterface` ([PdiStack.cpp](src/PdiStack.cpp)), per-session SSH buffers, controller HTML scratch | `new` / `pdiutil` containers |
...
|---|---|---|---|
| `RODT_ATTR("text")` | `devices/<board>/<board>.h` | Expands to `(const char*)F("text")` — wraps the literal in the Arduino `F()` macro so it stays in flash | Falls through to the default `#define RODT_ATTR(x) x` in [DataTypeDef.h:?](src/utility/DataTypeDef.h) — plain literal, identical behaviour |
...
| `+ ENABLE_NAPT` | small flash | **+10-20 KB heap** | n/a | lwIP NAPT table; the biggest hidden cost on a "small" build |
| `+ ENABLE_CONTEXTUAL_EXECUTION` | ~+10 KB | small | +`stackdepth` bytes per cooperative/preemptive task | Plus timer ISR for preemption |
...
- **Contextual execution** is cheap in flash but the per-task stacks live in RAM for the lifetime of the task. Two cooperative tasks at 1 KB each is 2 KB of RAM you can't reclaim.
...
1. **Don't `new` after `setup()`.** The single `m_client = new TcpClientInterface` in [PdiStack.cpp](src/PdiStack.cpp) is acceptable because it happens once at static-init. Anything that allocates per request, per tick, or per event will fragment.
...
power-on
 ...
 ├─ PDIStack ctor: m_client = new TcpClientInterface ~1 ms
setup()
...
Inside `#ifdef ENABLE_CONTEXTUAL_EXECUTION` (esp8266 only by default), two more tasks demonstrate the contextual lanes — each is a `while(1) { … sleep(500); }` body promoted via `scheduleUnderExecSched` with a 1 KB per-task stack:
...
1. `devices/<board>/` folder with the umbrella header and aggregator `.h` / `.cpp`.
...
3. Branch on `DEVICE_<NAME>` in [src/interface/pdi.h](src/interface/pdi.h).
...
7. Iterate on optional interfaces (network, storage, WiFi, threading) one flag at a time.
...
**Q. How do I add TLS?**
At the **port** layer — provide an `iClientInterface` implementation that wraps the vendor SDK's TLS API. Every transport (HTTP, MQTT, SMTP) accepts any `iClientInterface*`, so they get TLS for free once the port exposes one. The framework doesn't ship a portable TLS stack.
