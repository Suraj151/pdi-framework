...
| Built-in commands | `help`, `uptime`, `ls`, `cd`, `pwd`, `mkd`, `mkf`, `mv`, `cp`, `rm`, `cat`, `fwrite`, `head`, `tail`, `wc`, `df`, `grep`, `hexdump`, `cls`, `gpio`, `net`, `srvc`, `scht`, `ssh`, `iot`, `auth`, `reboot`, `watch` — full reference in [README Terminal table](#terminal) and [§10](#10-command-line--terminal) |
